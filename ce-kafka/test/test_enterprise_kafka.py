...
class ConfigTest(unittest.TestCase):
    ...
    def test_adb_config(self):
        ...
        zk_props = self.cluster.run_command_on_service("adb-metrics", "bash -c 'cat /etc/kafka/kafka.properties | sort'").decode()
        expected = """
        ...
        self.assertTrue("PASS" in self.cluster.run_command_on_service("adb-metrics", TOPIC_EXISTS_CHECK.format(topic="_confluent-metrics")).decode())
...
class ClusterHostNetworkTest(unittest.TestCase):
    ...
    def test_adb(self):
        ...
        topic_output = self.cluster.run_command_on_service("kafka-1", TOPIC_CREATE.format(topic=topic, partitions=10, replicas=3)).decode()
        assert 'Created topic "%s".' % topic in topic_output

        record_count = 100000
        produce_data_output = self.cluster.run_command_on_service("kafka-1", GENERATE_PERF_DATA.format(brokers="localhost:19092", topic=topic, record_count=record_count, record_size_bytes=1000, throughput_rps=100000)).decode()
        assert "%s records sent" % record_count in produce_data_output

        proposed_assignment_output = self.cluster.run_command_on_service("kafka-1", ADB_PROPOSED_ASSIGNMENT.format(brokers="localhost:19092")).decode()
        ...
        rebalance_status = self.cluster.run_command_on_service("kafka-1", ADB_STATUS).decode()

        rebalance_complete = ""
        for i in xrange(120):
            rebalance_complete = self.cluster.run_command_on_service("kafka-1", ADB_FINISH).decode()
