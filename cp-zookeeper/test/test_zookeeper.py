...
class ClusterBridgeNetworkTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        local_secrets_dir = os.path.join(FIXTURES_DIR, "secrets")
        utils.run_cmd("cp -rf {0} {1}".format(local_secrets_dir, "/tmp/zookeeper-bridged-test"))
    ...
    def tearDownClass(cls):
        cls.cluster.shutdown()
        utils.run_cmd("rm -rf /tmp/zookeeper-bridged-test")
...
class ClusterHostNetworkTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        local_secrets_dir = os.path.join(FIXTURES_DIR, "secrets")
        utils.run_cmd("cp -rf {0} {1}".format(local_secrets_dir, "/tmp/zookeeper-host-test"))
    ...
    def tearDownClass(cls):
        cls.cluster.shutdown()
        utils.run_cmd("rm -rf /tmp/zookeeper-host-test")
